...
export function generateStaticParams() {
  return ELDER_CARE_PACKAGES.map((pkg) => ({ packageId: pkg.id }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { packageId } = await params;
  const pkg = getElderCarePackage(packageId);
...
export default async function ElderCarePackagePage({ params }: PageProps) {
  const { packageId } = await params;
  const pkg = getElderCarePackage(packageId);
  if (!pkg) notFound();
  ...
  const schemas = [
    ...
    getServiceSchema({
      name: `${pkg.name} Elder Care Package`,
      description: `${pkg.tagline}. ${pkg.bestFor.join(' ')}`,
      path: pkg.href,
      serviceType: 'Elder care package',
    }),
  ];
    ...
    <>
        ...
        <div className="container elder-package-hero__grid">
          <div>
            <div className="elder-package-hero__badge">
              <SiteIcon name={pkg.icon} size={18} /> Elder Care Package
            </div>
            <h1>{pkg.name}</h1>
            <p>{pkg.tagline}</p>
            ...
            <div className="elder-package-hero__facts">
              ...
              <span><SiteIcon name="Check" size={16} /> Nurse oversight</span>
              <span><SiteIcon name="Check" size={16} /> Family updates</span>
            </div>
          </div>
          <div className="elder-package-hero__media">
            <Image src={pkg.image} alt={`${pkg.name} elder care support`} fill sizes="(max-width: 992px) 100vw, 40vw" priority />
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container elder-detail-grid">
          ...
          <aside className="elder-detail-cta" id="elder-package-form" style={packageStyle(pkg)}>
            <CTAForm
              title={`Book ${pkg.name} - Elder Care`}
              packageName={pkg.name}
              packageOptions={ELDER_CARE_PACKAGE_NAMES}
            />
          </aside>
        </div>
      </section>
      ...
      <section className="section section--alt">
        <div className="container elder-value-layout">
          ...
          <div className="elder-value-card" style={packageStyle(pkg)}>
            ...
            <h3>Transparent family reporting</h3>
            <p>Daily logs, WhatsApp updates, escalation notes, and supervisor check-ins help families stay close to the care even when they cannot be physically present.</p>
            <Link href="#elder-package-form" className="btn btn--white">Book {pkg.name}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <div className="section__header">
            ...
            <h2>Explore Other Elder Care Packages</h2>
          </div>
